...
public:
  ...
  enum Colors {
    ...
    COLOR_GREEN   = RGB(0X0D, 0XFF, 0X0D),
  ...
  uint32_t run() {
    ...
    const long t = millis();
    if (g_buttonPressed) {
      if (t - g_buttonPressTime > BUTTON_HOLD_TIME_ACTION)     { return beatLED(COLOR_BLUE,   (int[]){ 100, 100 }); }
      if (t - g_buttonPressTime > BUTTON_HOLD_TIME_INDICATION) { return waveLED(COLOR_BLUE,   1000); }
    }
    switch (currState) {
    case MODE_RESET_CONFIG:
    case MODE_WAIT_CONFIG:       return beatLED(COLOR_RED,    (int[]){ 50, 500 });
    case MODE_CONFIGURING:       return beatLED(COLOR_RED,    (int[]){ 200, 200 });
    ...
    case MODE_RUNNING:           return waveLED(COLOR_GREEN,   5000);
