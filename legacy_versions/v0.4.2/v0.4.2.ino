...
int umbral = 20;          // Cuando el nivel de agua caiga por debajo de este umbral(%) se activará la alarma
bool alarma = 0;          // Variable que activará o desactivará la alarma
bool q = 1;               // Esta variable hará que el zumbador prenda y apague cuando se activa la alarma
...
double total = 11;              // Esta variable es el valor maximo de profundidad del tanque donde se va a utilizar el sensor de presion
...
double tol=-0.0995;             // Calibracion para la intercepcion con el eje y
...
double lon = -98.77083;               // Longitud en valor decimal
double lat = 20.0625;                 // Latitud en valor decimal
...
BlynkTimer timer;                     // Esta función crea el objeto timer para enviar datos al servidor. Es parte de la librería Blynk
BlynkTimer timer1;                    // Esta función crea el objeto timer1 para tomar datos del sensor de presión.
BlynkTimer timer2;                    // Esta función crea el objeto timer2 para mostrar los datos en la pantalla con cada tick
BlynkTimer timer3;                    // Esta función crea el objeto timer3 para hacer sonar la alarma cuando se active
...
void Medicion()
{
    ...
    if(n==200){           
      ...
      Level = (slope_h2o * Vh2o - slope_air * Vair)/Vs + tol;  //Cálculo de la profundidad con los valores medidos y las constantes de calibración
...
void Pantalla(){
  ...
  if (digitalRead(boton) == 1){             //Verifica si el botón se ha pulsado y cambia el estado lógico de la variable 'modo'
    modo = !modo; 
  }
...
void Alarma(){
  //Con esta función se activará la alarma cuando la variable 'alarma' valga 1
  if(alarma == 1){
    if (q == 1){
        ledcWrite(ledChannel4, 440);             // Cuando q vale 1 el zumbador se activará durante el intervalo de tiempo del timer
    }
    else{
        ledcWrite(ledChannel4, 0);               // Cuando q vale 0 el zumbador se desactivará durante el intervalo de tiempo del timer
    }
  }
  q = !q;                                      // La variable q cambia de estado con cada ciclo para que el zumbador prenda y apague continuamente
}
...
void setup()
{
  ...
  timer3.setInterval(500L, Alarma);    // Cada 500 ms se corre la función Alarma, que se active el zumbador dependerá del valor de la variable 'alarma'. 
}
...
void loop() {
  ...
  while (digitalRead(rst) == 0){
    ...
    lcd.print("por ");
    for(int i=10;i>=0;i--){
      if(digitalRead(rst)==1){break;}    // Si se deja de pulsar el botón se interrumpe el contador
      lcd.setCursor(4,1);
      lcd.print(i);
      lcd.print(" segundos ");
      delay(1000);
