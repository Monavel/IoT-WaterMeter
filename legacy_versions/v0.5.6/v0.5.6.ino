...
#define BLYNK_TEMPLATE_ID "TMPL2SCEtifS_"
...
double total = 4.5;              // Esta variable es el valor maximo de profundidad del tanque donde se va a utilizar el sensor de presion
...
int    n = 200;                   // Este es el numero de muestras que se van a tomar para calcular el promedio
...
LiquidCrystal_I2C lcd(0x27, 20, 4);   //Declaracion del objeto para la pantalla. Las direcciones para la pantalla pueden ser 0x27 o 0x3f o 0x20 o 0x38 (depende del fabricante)
...
TaskHandle_t Mediciones;
//TaskHandle_t Reset;
void setup()
{
  Serial.begin(115200);                     //Se define la velocidad de la comunicación serial del ESP32
  xTaskCreatePinnedToCore(loop0,"Mediciones",3000,NULL,1,&Mediciones,1);
  //xTaskCreatePinnedToCore(loop1,"Reset",5000,NULL,1,&Reset,1);
...
BLYNK_CONNECTED() {
  ...
  lcd.setCursor(0,1);
  lcd.print("exito!");
  //Serial.println("Proceso en EL core: " + String(xPortGetCoreID()));
  delay(2000);
}
...
void loop0(void *parameter){
  while(1){
    //Serial.println("Proceso en NUCLEO: " + String(xPortGetCoreID()));
    ...
    for(int i=10;i>=0;i--){
      ...
      if (i==0){
        lcd3.clear();
        lcd3.setCursor(0, 0);
        lcd3.print("Reseteando...");
        lcd3.setCursor(0,1);
        lcd3.print("Suelte el boton");
        delay(3000);
      }
      else{
        lcd.setCursor(4,1);
